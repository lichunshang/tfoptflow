...
class ModelPWCNet(ModelBase):
    ...
    def build_model_towers(self):
        ...
        with tf.variable_scope(tf.get_variable_scope()) as outer_scope:
            for n, ops_device in enumerate(self.opts['gpu_devices']):
                print("  Building tower_{}...".format(n))
                # Use the assign_to_device function to ensure that variables are created on the controller.
                with tf.device(assign_to_device(ops_device, self.opts['controller'])), tf.name_scope('tower_{}'.format(n)):
    ...
    def config_loggers(self):
        ...
        """
        super(ModelPWCNet, self).config_loggers()
            ...
            while step < self.opts['max_steps'] + 1:
                ...
                if step % self.opts['display_step'] == 0:
                    ...
                    if self.opts['verbose']:
                        ...
                        status = "{} Iter {}".format(ts, self.g_step_op.eval(session=self.sess)) + \
                                 " [Train]: loss={:.2f}, epe={:.2f}, ={:.6f},".format(loss, epe, lr) + \
                                 " samples/sec={:.1f}, sec/step={:.3f},".format(samples_per_sec, sec_per_step) + \
                                 " eta={}".format(datetime.timedelta(seconds=eta))
                        print(status)
                ...
                if val_batch_size > 0 and step % self.opts['val_step'] == 0:
                    ...
                    if self.opts['verbose']:
                        ts = time.strftime("%Y-%m-%d %H:%M:%S")
                        status = "{} Iter {} [Val]: loss={:.2f}, epe={:.2f}".format(ts, self.g_step_op.eval(session=self.sess), loss, epe)
                        print(status)
    ...
    def eval(self, metric_name=None, save_preds=False):
        ...
        with self.graph.as_default():
            ...
            desc = 'Measuring {} and saving preds'.format(metric_name) if save_preds else 'Measuring {}'.format(metric_name)
            idx = 0
            for _round in trange(rounds, ascii=True, ncols=100, desc=desc):
                ...
                for y_hat, metric, y_hat_path, ID in zip(y_hats, metrics, y_hat_paths, IDs):
                    ...
                    if save_preds:
                        ...
                        info="{}={:.2f}".format(metric_name, metric)
    ...
    def refine_flow(self, feat, flow, lvl, name='ctxt'):
        ...
        Ref:
            ...
            7 convolutional layers. The spatial kernel for each convolutional layer is 3x3. These layers have different
        ...
        with tf.variable_scope(name):
            ...
            for lvl in range(self.opts['pyr_lvls'], self.opts['flow_pred_lvl'] - 1, -1):
                ...
                if lvl != self.opts['flow_pred_lvl']:
                    ...
                    if self.dbg:
                        print('Upsampling {} by {} in each dimension.'.format(flow.op.name, scaler))
